...
BYTE MMCWRData[MMC_DATA_SIZE]={"  ARM7 LPC2148  "};
...
int mmc_write_block(WORD block_number)
{
 ...
 BYTE Status;
 IOCLR0 = SPI_SEL; /* clear SPI SSEL */
 varl=((block_number&0x003F)<<9);		/* block size has been set in mmc_init() */
 varh=((block_number&0xFFC0)>>7);
 
 MMCCmd[0] = 0x58;						/* send mmc CMD24(WRITE_SINGLE_BLOCK) to write the data to MMC card */
 MMCCmd[1] = varh >> 0x08;				/* high block address bits, varh HIGH and LOW */
 MMCCmd[2] = varh & 0xFF;
 MMCCmd[3] = varl >> 0x08;				/* low block address bits, varl HIGH and LOW */
 MMCCmd[4] = varl & 0xFF;
 MMCCmd[5] = 0xFF;						/* checksum is no longer required but we always send 0xFF */
 SPI_Send(MMCCmd, MMC_CMD_SIZE );
